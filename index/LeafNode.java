...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int cap = 2*metadata.getOrder();
        int sizeAfterAdd = keys.size() + 1;
        keys.add(key);
        ...
        if(sizeAfterAdd <= cap) {
            ...
            LeafNode newRightLeafNode = new LeafNode(metadata, bufferManager, keys.subList(cap/2, cap+1),
                    rids.subList(cap/2, cap+1), this.rightSibling, treeContext);
            DataBox split_key = keys.get(cap/2);
            long right_node_page_num = newRightLeafNode.getPage().getPageNum();
            this.rightSibling = Optional.of(right_node_page_num);
            keys = keys.subList(0, cap/2);
            rids = rids.subList(0, cap/2);
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while(data.hasNext() && keys.size()<cap+1){
            Pair<DataBox, RecordId> newPair = data.next();
            keys.add(newPair.getFirst());
            rids.add(newPair.getSecond());
        }
        if(keys.size() == cap+1) {
            LeafNode newRightLeafNode = new LeafNode(metadata, bufferManager, keys.subList(cap, cap+1),
                    rids.subList(cap, cap+1), Optional.empty(), treeContext);
            DataBox split_key = keys.get(cap);
            long right_node_page_num = newRightLeafNode.getPage().getPageNum();
            this.rightSibling = Optional.of(right_node_page_num);
            keys = keys.subList(0, cap);
            rids = rids.subList(0, cap);
            sync();
            return Optional.of(new Pair<>(split_key, right_node_page_num));
        }
        sync();
    ...
    public void remove(DataBox key) {
        ...
        if(keys.contains(key)) {
            int removed_index = keys.indexOf(key);
            keys.remove(key);
            rids.remove(removed_index);
            sync();
        }
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        if(rightSiblingPageNum == -1) rightSibling = Optional.empty();
        else rightSibling = Optional.of(rightSiblingPageNum);
        ...
        for (int i=0; i < length; i++) {
            ...
            Long pNum = buf.getLong();
            Short eNum = buf.getShort();
            rids.add(new RecordId(pNum, eNum));
        }
