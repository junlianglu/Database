...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        for(int i=0; i<children.size(); i++) {
            if(i == 0) {
                ...
                if(keys.get(i-1).compareTo(key)<=0 && key.compareTo(keys.get(i))<0) {
                    foundIndex = i;
                    break;
                }
            } else if(i == keys.size()) {
        ...
        if(foundIndex >= 0) {
            BPlusNode bplusnode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(foundIndex));
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if(foundIndex >= 0) {
            BPlusNode bplusnode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(foundIndex));
            Optional<Pair<DataBox, Long>> p = bplusnode.put(key, rid);
            if(!p.isPresent()) {
                sync();
                return Optional.empty();
            }
            DataBox split_key_from_child = p.get().getFirst();
            Long right_node_page_num_from_child = p.get().getSecond();

            keys.add(split_key_from_child);
            Collections.sort(keys);
            children.add(keys.indexOf(split_key_from_child)+1, right_node_page_num_from_child);
            if(keys.size() <= cap) {
                sync();
                return Optional.empty();
            } else {
                InnerNode newRightInnerNode = new InnerNode(metadata, bufferManager, keys.subList(cap/2+1, cap+1),
                        children.subList(cap/2+1, cap+2), treeContext);
                DataBox inner_split_key = keys.get(cap/2);
                long right_inner_node_page_num = newRightInnerNode.getPage().getPageNum();
                keys = keys.subList(0, cap/2);
                children = children.subList(0, cap/2+1);
                sync();
                return Optional.of(new Pair<>(inner_split_key, right_inner_node_page_num));
            }
        }
        throw new BPlusTreeException("Impossible");
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int d = metadata.getOrder();
        int cap = 2*d;
        Long rightMostChildPageNum = children.get(children.size()-1);
        BPlusNode bplusnode = BPlusNode.fromBytes(metadata, bufferManager, treeContext, rightMostChildPageNum);
        Optional<Pair<DataBox, Long>> p = bplusnode.bulkLoad(data, fillFactor);
        if(!p.isPresent()) {
            sync();
            return Optional.empty();
        }
        ...
        keys.add(split_key_from_child);
        children.add(right_node_page_num_from_child);
        if(keys.size() <= cap) {
            sync();
