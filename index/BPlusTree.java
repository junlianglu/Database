...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode target_leaf = root.get(key);
        List<DataBox> target_leaf_keys = target_leaf.getKeys();
        List<RecordId> target_leaf_rids = target_leaf.getRids();
        if(root instanceof LeafNode) return null;
        if(target_leaf_keys.contains(key)) {
            return Optional.of(target_leaf_rids.get(target_leaf_keys.indexOf(key)));
        }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        if(root instanceof LeafNode && ((LeafNode) root).getRids().size() == 0)
            return Collections.emptyIterator();
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        newKeys.add(split_key_from_child);
        newChildren.add(root.getPage().getPageNum());
        newChildren.add(right_node_page_num_from_child);
        InnerNode newtRoot = new InnerNode(metadata, bufferManager, newKeys,
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if(!(root instanceof LeafNode)
                || ((LeafNode) root).getKeys().size()!=0 || ((LeafNode) root).getRids().size()!=0)
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            return index < currLeaf.getKeys().size();
        }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            if(hasNext()) {
                RecordId rid = currLeaf.getRids().get(index);
                index++;
                if(index >= currLeaf.getKeys().size() && currLeaf.getRightSibling().isPresent()) {
                    index = 0;
