...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        void grantOrUpdateLock(Lock lock) {
            ...
            if(!transactionLocks.containsKey(lock.transactionNum)) {
                List<Lock> xact_locks = new ArrayList<>();
                xact_locks.add(lock);
                transactionLocks.put(lock.transactionNum, xact_locks);
                this.locks.add(lock);
            }else if(transactionLocks.get(lock.transactionNum).isEmpty()) {
                transactionLocks.get(lock.transactionNum).add(lock);
                this.locks.add(lock);
            }else {
                // check if it is upgrade, if upgrade then upgrade and return
                for(Lock xact_lock : transactionLocks.get(lock.transactionNum)) {
                    if(xact_lock.name.equals(lock.name)) {
                        xact_lock.lockType = lock.lockType;
                        assert locks.contains(xact_lock);  // Check resource locks changed too
                        return;
                    }
                }
                // is grant
        ...
        private void processQueue() {
            ...
            while(!waitingQueue.isEmpty()) {
                LockRequest nextRequest = waitingQueue.getFirst();
                if(checkCompatible(nextRequest.lock.lockType, nextRequest.lock.transactionNum)) {
                    waitingQueue.poll();
                    grantOrUpdateLock(nextRequest.lock);
                    for(Lock lockThatNeedsToBeReleased : nextRequest.releasedLocks) {
                        release(nextRequest.transaction, lockThatNeedsToBeReleased.name);
                    }
    ...
    throws DuplicateLockRequestException, NoLockHeldException {
        ...
        synchronized (this) {
            ...
            // error checking and build list for releasing locks
            for(ResourceName resourceName : releaseLocks) {
                boolean noLockHeld = true;
                for(Lock resourceLock : getResourceEntry(resourceName).locks) {
                    if(resourceLock.transactionNum == xactNum) {
                        noLockHeld = false;
                        releasedLocks.add(new Lock(resourceName, resourceLock.lockType, xactNum));
                    }
                }
                if(noLockHeld) {
            ...
            if(entry.getTransactionLockType(xactNum) != LockType.NL) {
                if(!releasedLocks.contains(new Lock(name, entry.getTransactionLockType(xactNum), xactNum))) {
                    throw new DuplicateLockRequestException("Duplicate lockRequest not allowed!");
                }else {
                    // for promoteAndRelease phase
                    releasedLocks.remove(new Lock(name, entry.getTransactionLockType(xactNum), xactNum));
                }
            }

            // acquireAndRelease phase
            Lock requestedLock = new Lock(name, lockType, xactNum);
            if(entry.checkCompatible(lockType, xactNum)) {
                entry.grantOrUpdateLock(requestedLock);
                for(Lock lockToRelease : releasedLocks) {
                    getResourceEntry(lockToRelease.name).releaseLock(lockToRelease);
                }
            }else {
                shouldBlock = true;
                entry.addToQueue(new LockRequest(transaction, requestedLock, releasedLocks), true);
            }
        }
        if(shouldBlock) {
            transaction.block();
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            Lock requestedLock = new Lock(name, lockType, xactNum);
            if(entry.waitingQueue.isEmpty() && entry.checkCompatible(lockType, xactNum)) {
                entry.grantOrUpdateLock(requestedLock);
            }else {
                shouldBlock = true;
                entry.addToQueue(new LockRequest(transaction, requestedLock), false);
            }
        }
        if(shouldBlock) {
            transaction.block();
    ...
    throws NoLockHeldException {
        ...
        synchronized (this) {
            ...
            for(Lock lock : entry.locks) {
                if(lock.transactionNum == xactNum) {
                    noLockHeld = false;
                    targetLock = lock;
                    break;
                }
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        ...
        synchronized (this) {
            ...
            if(newLockType == LockType.SIX) {
                throw new InvalidLockException("don't use method promote to promote to SIX !");
            }
            // throws DuplicateLockRequestException
            if(entry.locks.contains(new Lock(name, newLockType, xactNum))) {
            ...
            if(newLockType==targetLock.lockType || !LockType.substitutable(newLockType, targetLock.lockType)) {
                throw new InvalidLockException("not a promotion!");
            }

            Lock updatedLock = new Lock(name, newLockType, xactNum);
            if(entry.checkCompatible(newLockType, xactNum)) {
                entry.grantOrUpdateLock(updatedLock);
            }else {
                shouldBlock = true;
                entry.addToQueue(new LockRequest(transaction, updatedLock), true);
            }
        }
        if(shouldBlock) {
            transaction.block();
