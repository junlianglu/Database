...
public class SortOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        while(!q.isEmpty()) {
            ...
            int iter_id = p.getSecond();
            newRun.addRecord(p.getFirst().getValues());
            if(iters.get(iter_id).hasNext()) {
                q.add(new Pair<>(iters.get(iter_id).next(), iter_id));
            }
        }
        return newRun;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int given_num_of_sorted_runs = runs.size();
        int new_num_of_sorted_runs = (int)Math.ceil(given_num_of_sorted_runs/(numBuffers-1));
        if(given_num_of_sorted_runs == new_num_of_sorted_runs) {
            return runs;
        }else {
            for(int i=0; i<new_num_of_sorted_runs; i++) {
                if((i+1)*(numBuffers-1)>given_num_of_sorted_runs) {
                    Run newRun = mergeSortedRuns(runs.subList(i*(numBuffers-1), given_num_of_sorted_runs));
                    newRuns.add(newRun);
                }else {
                    Run newRun = mergeSortedRuns(runs.subList(i*(numBuffers-1), (i+1)*(numBuffers-1)));
                    newRuns.add(newRun);
                }
    ...
    public String sort() {
        ...
        BacktrackingIterator<Page> piter = transaction.getPageIterator(tableName);
        while(piter.hasNext()) {
            BacktrackingIterator<Record> riter = transaction.getBlockIterator(tableName, piter, numBuffers);
            Run run = createRunFromIterator(riter);
