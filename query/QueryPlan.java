...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Set singleTableSet = new HashSet();
        ...
        for(String joinTableName : joinTableNames) {
            Set tempSingleTableSet = new HashSet();
            tempSingleTableSet.add(joinTableName);
            pass1Map.put(tempSingleTableSet, minCostSingleAccess(joinTableName));
        }
        assert pass1Map.containsKey(singleTableSet);
        ...
        startSet.add(this.startTableName);
        prevMap.put(startSet, minCostSingleAccess(this.startTableName));
        for(String joinTableName : joinTableNames) {
            Set tempSingleTableSet = new HashSet();
            tempSingleTableSet.add(joinTableName);
            prevMap.put(tempSingleTableSet, minCostSingleAccess(joinTableName));
        }*/
        int maxJoinedTableSize = joinTableNames.size() + 1;
        int currPass = 2;
        while(currPass <= maxJoinedTableSize) {
     ...
     * the minimum cost operation. Then push down eligible selects (SELECT
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for(int i=0; i<selectIndices.size(); i++) {
            int indexOfCurrIndexColumn = selectIndices.get(i);
            QueryOperator currIndexOp = new IndexScanOperator(this.transaction, table,
                    this.selectColumnNames.get(indexOfCurrIndexColumn), this.selectOperators.get(indexOfCurrIndexColumn),
                    this.selectDataBoxes.get(indexOfCurrIndexColumn));
            if(currIndexOp.estimateIOCost() < minOpCost) {
                minOp = currIndexOp;
                minOpCost = currIndexOp.estimateIOCost();
                except = indexOfCurrIndexColumn;
            }
        }
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for(Set tables : prevMap.keySet()) {
            ...
            for(int i=0; i<this.joinLeftColumnNames.size(); i++) {
                String[] leftJoinTableNameAndColumnName = getJoinLeftColumnNameByIndex(i);
                String leftJoinTableName = leftJoinTableNameAndColumnName[0];
                String leftJoinColumnName = leftJoinTableNameAndColumnName[1];
                String[] rightJoinTableNameAndColumnName = getJoinRightColumnNameByIndex(i);
                String rightJoinTableName = rightJoinTableNameAndColumnName[0];
                String rightJoinColumnName = rightJoinTableNameAndColumnName[1];
                if(tables.contains(leftJoinTableName) && !tables.contains(rightJoinTableName)) {
                    QueryOperator rightOp = null;
                    Set singleTableSet = new HashSet();
                    singleTableSet.add(rightJoinTableName);
                    rightOp = pass1Map.get(singleTableSet);
                    Set newJoinedTables = new HashSet(tables);
                    newJoinedTables.add(rightJoinTableName);
                    QueryOperator minOp = minCostJoinType(leftOp, rightOp, leftJoinColumnName, rightJoinColumnName);
                    /*QueryOperator leftFirstMinOp = minCostJoinType(leftOp, rightOp, leftJoinColumnName, rightJoinColumnName);
                    QueryOperator rightFirstMinOp = minCostJoinType(rightOp, leftOp, rightJoinColumnName, leftJoinColumnName);
                    QueryOperator minOp = null;
                    if(rightFirstMinOp.estimateIOCost() <= leftFirstMinOp.estimateIOCost()) {
                        minOp = rightFirstMinOp;
                    }else{
                        minOp = leftFirstMinOp;
                    }
                    assert  minOp != null;*/
                    if(map.containsKey(newJoinedTables)) {
                        if(minOp.estimateIOCost() < map.get(newJoinedTables).estimateIOCost()) {
                            map.replace(newJoinedTables, minOp);
                        }
                    }else{
                        map.put(newJoinedTables, minOp);
                    }
                }else if(tables.contains(rightJoinTableName) && !tables.contains(leftJoinTableName)) {
                    QueryOperator rightOp = null;
                    Set singleTableSet = new HashSet();
                    singleTableSet.add(leftJoinTableName);
                    rightOp = pass1Map.get(singleTableSet);
                    Set newJoinedTables = new HashSet(tables);
                    newJoinedTables.add(leftJoinTableName);
                    QueryOperator minOp = minCostJoinType(leftOp, rightOp, rightJoinColumnName, leftJoinColumnName);
                    /*QueryOperator leftFirstMinOp = minCostJoinType(leftOp, rightOp, rightJoinColumnName, leftJoinColumnName);
                    QueryOperator rightFirstMinOp = minCostJoinType(rightOp, leftOp, leftJoinColumnName, rightJoinColumnName);
                    QueryOperator minOp = null;
                    if(rightFirstMinOp.estimateIOCost() <= leftFirstMinOp.estimateIOCost()) {
                        minOp = rightFirstMinOp;
                    }else{
